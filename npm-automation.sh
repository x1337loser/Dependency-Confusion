...
waybackurls $1 | sort -u | grep .js | sed 's/?.*//' | grep -v '/wp-content/\|/wp-includes/\|.json\|jpg\|png\|css|\|/member/\|.jsp\|oauth\|login\|en-us\|v=\|=\|?\|/help/\|/id/\|paragon\|/wp-json/' | sort -u | tee -a $HOME/$1/$1-js-urls.txt >/dev/null 2>&1;
echo -e "Running gau on $1"
gau $1 | sort -u | grep .js | sed 's/?.*//' | grep -v '/wp-content/\|/wp-includes/\|.json\|jpg\|png\|css|\|/member/\|.jsp\|oauth\|login\|en-us\|v=\|=\|?\|/help/\|/id/\|paragon\|/wp-json/' | sort -u | tee -a $HOME/$1/$1-js-urls.txt >/dev/null 2>&1;
...
for val in $num;do
    grep -oriahE "[^\"\\'> ]+" | grep 'node_modules' | grep -v '@' | cut -d '/' -f"$val" | sort -u | grep -v '.js\|.ts\|.tsx\|.css' | egrep '\b[a-z]+\b' | grep -v '.png\|.pnp' | tee -a $1-npm-packages.txt >/dev/null 2>&1;
    done
...
if [ -s $1-npm-packages.txt ];then
    ...
    cat $1-npm-packages.txt | sort -u | while read ut;do
        if $(curl -o /dev/null -s -w "%{http_code}\n" "https://registry.npmjs.org/$ut" | grep "404" >/dev/null 2>&1); then
            ...
            echo -e ""$ut" Availible in Public Registry ";
        fi
        done
else
...
for val in $num;do
    grep -oriahE "[^\"\\'> ]+" | grep 'node_modules' | cut -d '/' -f"$val" | sort -u | grep '@' | grep -v '.js\|.ts\|.tsx\|.css' | egrep '\b[a-z]+\b' | grep -v '.png\|.pnp' | grep '@' | cut -d '@' -f 2 | tee -a $1-npm-scope.txt >/dev/null 2>&1;
    done
if [ -s $1-npm-scope.txt ];then
    ...
    OPTION=`curl -o /dev/null -s -w "%{http_code}\n" "https://www.npmjs.com/org/$pkg"`
        if $(echo "$OPTION" | grep "200\|302" >/dev/null 2>&1);then
            echo -e ""$pkg" Availible in Public Registry " && echo $pkg >> $1-npm-scope-vuln.txt;
